...
160M-parameter pretraining pilot (Pythia-160M, two seeds, through the induction formation window) tests transfer,
and the ban/selection side transfers cleanly: the imag-share ban again drives the previous-token head to a cos-only
implementation (\rif${\approx}0.005$), rerouted to a different, non-seeded site, at a formation delay of
$+33$--$50\%$ that lands in the toy range and is sign-consistent across seeds, so the non-default implementation
stays reachable and solution degeneracy does not shrink at scale. Two honest corrections travel with this. First,
at real-corpus scale the ban is \emph{not} capability-free: general language-model learning is also delayed
(validation crossings $+8$--$23\%$; endpoint gap ${\approx}{+}0.04$--$0.05$ nats), so the toy report of an
unaffected LM floor overstates, and the circuit-specific delay is the part above this general slowdown. Second, the
assist/acceleration arm does \emph{not} cleanly test transfer: the frozen plant we used was behaviorally inert at
160M, never functioning as a previous-token head (seeded attention stayed at the uniform baseline), and it was
handicapped by freezing a slot that is itself the natural previous-token winner; the pilot therefore establishes
only that this plant construction fails to install a functional head at 160M, not whether initialization can
accelerate at scale, which a scale-adequate plant would settle. Weak-regularizer selection also remains untested at
scale.
...
(a toy-scale result; whether it transfers is not yet cleanly tested, our 160M assist arm being confounded by a
behaviorally inert plant, see the caveats above). On the side that does transfer, our 160M pilot finds the
ban-induced selection and its cost carry over with solution degeneracy intact, so ``which algorithm a circuit
implements'' is at least partly a controllable design choice, relevant to interpretability-by-construction and to
