...
zero). These results are an existence demonstration of steerability at toy scale, not a scaling law; the
program's next milestone tests assisted selection at $\ge$160M-parameter pretraining through the formation window.

\section{Takeaway}
The spectral economy of attention is not only measurable and priced (companion paper) but \emph{steerable}: weak,
ban-free assistance selects which functionally-equivalent spectral implementation a circuit adopts, at a cost far
below banning the alternative, and solution-specific initialization accelerates circuit formation outright. If this
holds at scale, ``which algorithm a circuit implements'' becomes a controllable design choice --- relevant to
interpretability-by-construction and to installing chosen (e.g., more auditable) implementations. Steering is
also \emph{auditable} where rerouting is not hideable: companion propagator-level work finds that implementations
which vanish from the weight fingerprint (\rif$\,{\approx}0.003$ after a phase ban) remain fully visible as
transient-reserve excess on the attention propagator ($31/31$ prev-carrying heads positive across all
implementations) --- the weight battery reads \emph{implementation}, the propagator battery reads
\emph{function}, so a steered or rerouted circuit cannot evade both. We release the harness, seeds, and per-run
tables.
